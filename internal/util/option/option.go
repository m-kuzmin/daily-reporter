...
// MustUnwrap panics if Option is `None`. Otherwise returns the value contained in `Some`.
...
// MustExpect panics if Option is `None`. Otherwise returns the value contained in `Some`.
func (o Option[T]) MustExpect(a any) T {
	if v, isSome := o.Unwrap(); isSome {
		return v
	}
...
// MustExpectf panics if Option is `None`. Otherwise returns the value contained in `Some`.
func (o Option[T]) MustExpectf(fstr string, a ...any) T {
	if v, isSome := o.Unwrap(); isSome {
		return v
	}

	panic(fmt.Sprintf(fstr, a...))
}
