...
func (s *AddAPIKey) PrivateTextMessage(message update.PrivateTextMessage) (Handler, []response.BotAction) {
	...
	case "/cancel":
		return &Root{}, []response.BotAction{response.SendMessageBuilder(message.Chat.ID, s.responses.Cancel)}
	default:
		client := github.NewClient(message.Text)

		login, err := client.Login()
		if err != nil {
			return s, []response.BotAction{response.SendMessageBuilder(message.Chat.ID, s.responses.BadAPIKey)}
		}

		return &Root{}, []response.BotAction{response.SendMessageBuilder(message.Chat.ID,
			fmt.Sprintf(s.responses.Success, login, login)).ParseModeHTML()}
	}
}

func (s *AddAPIKey) GroupTextMessage(message update.GroupTextMessage) (Handler, []response.BotAction) {
	return &Root{}, []response.BotAction{response.SendMessageBuilder(message.Chat.ID, s.responses.KeySentInPublicChat)}
}
